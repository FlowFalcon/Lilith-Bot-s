...
const procMsg = async (originalMessage, socket, dStore) => {
  ...
  const pushName =
    typeof rawPushName === "string" && rawPushName ? rawPushName : "Unknown";
  const metadata =
  ...
  const isMedia = /imageMessage|videoMessage|stickerMessage|documentMessage|audioMessage/.test(
    msgType
  );
  ...
  if (isQuoted) {
    ...
    if (quotedMsgId) {
      quotedContent = Object.entries(dStore.messages)
        ...
        .find(
          (item) => item && typeof item === "object" && item.key?.id === quotedMsgId
        );
    }
  ...
  return {
    ...
    isBot,
    reply: async (text) => {
      return await socket.sendMessage(chatJid, { text }, { quoted: message });
    },
  };
};
...
module.exports = { procMsg, dlMedia };
